## **Relay Sample**
This sample will demonstrate how to use the [Grove Relay](http://www.seeedstudio.com/wiki/Grove_-_LED_Socket_Kit) with the and the Dexter Industries [GrovePi](http://www.dexterindustries.com/product-category/robots/grovepi/). This sample is designed to run on a Raspberry Pi 2 running Windows 10 IoT Core. The application is written as a Background Task and will run in Windows 10 IoT Core without any user interface.
...
7. Find the __Relay.csproj__ file - select it and click __Open__

This will add the _Relay_ project to you new _Samples_ directory.

In the _Relay_ project you need to update the reference to _GrovePi_.

1. In _Solution Explorer_ expand the _Relay_ project and the _References_ node.
...
1. Open __GrovePi\Software\CSharp\Samples\Relay.sln__ in Visual Studio.   
2. In _Solution Explorer_ expand the _Relay_ project and the _References_ node.  
...
1. Relay to __D5__ (Digital Pin 5)

### Run the Sample
To run the sample... 

1. Right-click on the _Relay_ project in the _Solution Explorer_
2. Select __Set as Startup Project__
3. Right-click on the __Properties__ node under _Relay_ and select __Open__
